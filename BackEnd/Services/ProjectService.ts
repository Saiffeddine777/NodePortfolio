...
export const removeOneProject: (id: number) => Promise<DeleteResult> = async (
  ...
  try {
    const projectToDelete: NullableOrUndefined<Project> = await ProjectRepository.findOneBy({id})
...
export const modifyOneProject: (
  ...
  try {
    if (file) {
      const projectToModify : Project |null = await ProjectRepository.findOneBy({id})
