...
describe("EmailRouter tests", () => {
  const reqBodyInput: Partial<Email> = {
    ...
    // In reqBodyInput.user AND mockResult.user — both need this added
    user: {
      id: 1,
      userName: "luna_code",
...
const mockResult: Email = {
  ...
  createdAt: new Date(),
  updatedAt: new Date(),
  ticket: {} as Ticket, // 👈 ADD THIS (or [] if it's an array relation)
};
  ...
  it("DELETE /api/emails/1 should delete one email with the id of 1", async () => {
    ...
    jest
      .spyOn(EmailController, "deleteOneEmail")
      .mockImplementation(async (req, res) => {
        res
          .status(200)
  ...
  it("POST /api/emails/ should create one email when given a body input", async () => {
    (verifyRecaptcha as jest.Mock).mockImplementation(
      (req: any, res: any, next: any) => next(),
    );
    jest
      .spyOn(EmailController, "postAnEMail")
      .mockImplementation(async (req, res) => {
        res.status(200).json(mockResult);
      });
  ...
  it("PUT /api/emails/1 should modify one email when given a body", async () => {
    ...
    jest
      .spyOn(EmailController, "putOneEmail")
      .mockImplementation(async (req, res) => {
        res
          .status(200)
